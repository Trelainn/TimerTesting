        ...
        if race_info['status'] == 'started':
            ...
            if race_info['category'] == 'Time race':
                if race_info['total_time'] > race_info['expectedTime']:
                    ...
                    race_info['message'] = str(requests.post("http://localhost:8080/race/", json={'stop_race': True, 'user_id': race_info['creator']}).status_code)
...
def participant():
    ...
    try:
        if status['race_status'] == 'configure_race':
            ...
            video_permission = False if user_id[:5].lower() == 'guest' else True
